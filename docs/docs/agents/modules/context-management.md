...
`model_name`. If the tokenizer cannot count the content, Datrix falls back to a
...
may quickly dominate the prompt. Datrix keeps the prompt compact by separating
...
If `AgentContext.output_dir` is set, Datrix uses that directory instead.
...
- Compaction is progressive: after each layer, Datrix recounts tokens and stops
