...
summary: "Shortest path from clone to a working Datrix chat"
read_when:
  - You want the first successful Datrix run with the least setup
...
:::info Fastest path
Use an **API proxy** (OpenAI or DeepSeek) — no GPU required. You will have a working Datrix chat in under 5 minutes.
:::
...
:::tip Verify it works
If the Web UI loads and you can start a chat conversation, your Datrix is ready for use.
:::
...
- **Web UI does not load**
  ...
  - Check the server logs in the terminal where you started Datrix
