...
Install Datrix from PyPI and start it with a single command — no source checkout required.
...
That's it! On first run Datrix will launch an **interactive setup wizard** that helps you:
...
Start the Datrix web server. Running `dbgpt start` without a subcommand is equivalent to `dbgpt start web`.
...
Stop running Datrix server processes.
...
After first run, Datrix creates the following structure under your home directory:
...
|---|---|---|
| `DBGPT_HOME` | `~/.dbgpt` | Override the Datrix home directory |
