...
Deploy Datrix with MySQL using Docker Compose — a production-ready setup with persistent storage.
...
The root `docker-compose.yml` deploys Datrix with a MySQL database and SiliconFlow as the default LLM provider.
...
| `webserver` | `eosphorosai/dbgpt-openai:latest` | 5670 | Datrix application server |
...
Datrix ships with additional Compose files for specific scenarios:
