...
- **SQLite users**: Back up your metadata database before upgrading. The new metadata tables are created automatically when the Datrix service starts.
...
### Step 1 — Stop Datrix Service

Stop the running Datrix service using the same method you used to start it.
...
### Step 4 — Start Datrix Service

Restart the Datrix service using your preferred start method.
...
If the upgrade fails, stop the Datrix service and restore the metadata database from the backup taken before the upgrade. Because v0.8.1 changes metadata schema, do not continue running a partially upgraded database.
