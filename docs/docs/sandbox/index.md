...
Datrix uses a sandbox to let agents execute code and tools in an isolated runtime
...
In Datrix, a sandbox is an isolated execution environment used by an agent when it
...
real environments. The sandbox gives Datrix a dedicated runtime for actions such
as:
...
## Datrix's current sandbox solution

Datrix's sandbox implementation lives in:
...
This allows Datrix to prefer container isolation when available and fall back to a
local execution mode for development or environments without container support.

## Layered architecture in `dbgpt-sandbox`

Datrix's sandbox is implemented as a small runtime system with several layers.
...
One important part of the Datrix sandbox design is that it supports **session-based
stateful execution**.
...
## Current integration in Datrix app

Today, Datrix already uses sandbox execution in application-side agent tooling.
...
## What Datrix supports today

Based on the current `dbgpt-sandbox` implementation, Datrix is moving toward a
...
## High-level view of the current Datrix sandbox direction
