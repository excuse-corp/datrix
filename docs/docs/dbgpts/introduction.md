...
In Datrix, a skill is a reusable capability package that gives an agent a structured way to solve a task.
...
In Datrix terms, a skill is not just “something the model knows.” It is a packaged workflow that helps the agent decide:
...
A Datrix skill package often includes:
...
Following the structure used by Datrix's own skill-creator guidance, a skill is organized as a small self-contained package:
