...
**Scheduled Tasks** turn a one-off conversation into a recurring job. Run a data analysis once, save it as a task, and Datrix replays the whole ReAct Agent flow on a cron schedule — generating a fresh report every time.
