...
Datrix 的数据源模块用于管理企业中的结构化和半结构化数据资产，把数据库、数仓、数据湖等系统接入到 Datrix 框架中，从而快速构建面向数据场景的智能应用和大模型能力。目前 Datrix 已支持多种常见数据源，也支持自定义扩展。
...
接入数据源之后，Datrix 可以基于这些结构化数据能力支持：
