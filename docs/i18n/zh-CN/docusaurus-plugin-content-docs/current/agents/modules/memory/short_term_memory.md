...
Like human short-term memory, the short-term memory in Datrix agents can be enhanced by outside observations.
...
If you have deployed [Datrix cluster](../../../installation/model_service/cluster) and
...
agent_memory: AgentMemory = AgentMemory(memory=memory)
```
In Datrix, the core interface is asynchronous and non-blocking, so we use `ThreadPoolExecutor` to
