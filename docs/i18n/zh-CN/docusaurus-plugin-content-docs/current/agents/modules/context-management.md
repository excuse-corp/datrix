...
工具观察结果可能非常大：SQL 结果表、生成代码输出、解释器日志、文件路径、报告元数据和中间计算值都可能快速占满 prompt。Datrix 通过把完整操作详情和需要进入模型上下文的文本拆开，来保持 prompt 紧凑。
...
如果设置了 `AgentContext.output_dir`，Datrix 会优先使用该目录。
