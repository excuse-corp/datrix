...
- `DatasourceResource`: A resource that can be used to connect to various data sources in Datrix.
It just works when you run your agent in the Datrix environment(running in the Datrix webserver).

In previous sections [Agents With Database](../../introduction/database), we have introduced
how to use the database resource in the Datrix agent, so you can refer to it for more details.
