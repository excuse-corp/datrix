...
Datrix 使用 sandbox（沙箱）让智能体在隔离的运行环境中执行代码和工具，而不是直接在宿主机环境中运行。
...
在 Datrix 中，sandbox 是一个隔离执行环境，供智能体在任务过程中执行代码、运行命令、处理文件或调用执行型工具。
...
如果智能体可以不受限制地直接执行代码，那么在真实环境中很难安全落地。Sandbox 为 Datrix 提供了一个专门的执行层，用于支持：
...
## Datrix 当前的 sandbox 方案

Datrix 当前的 sandbox 实现位于：
...
这意味着 Datrix 会优先使用容器隔离；如果部署环境没有容器支持，也可以退化到本地运行时用于开发或调试。
...
Datrix 当前 sandbox 设计的一个重要点，是支持**基于 session 的有状态执行**。
...
目前 Datrix 已经在应用侧 agent 工具里实际使用了 sandbox。
...
基于当前 `dbgpt-sandbox` 实现，Datrix 正在走向一个更通用的 agent 执行运行时，支持：
