...
通过 Docker Compose 搭配 MySQL 部署 Datrix。这种方式适合生产场景，并支持持久化存储。
...
根目录下的 `docker-compose.yml` 会部署一个带 MySQL 数据库的 Datrix，并默认使用 SiliconFlow 作为 LLM provider。
...
| `webserver` | `eosphorosai/dbgpt-openai:latest` | 5670 | Datrix 应用服务 |
...
Datrix 还提供了适用于不同场景的 Compose 文件：
