...
SMMF is Datrix's model management layer. It provides a unified interface for managing, switching, and deploying multiple LLM and embedding models — whether they are API proxies or locally hosted.
...
All models run in the same process as the Datrix server. Simple and suitable for development or single-machine deployments.
...
```mermaid
flowchart LR
    Server["Datrix Server"] --> Controller["Model Controller"]
