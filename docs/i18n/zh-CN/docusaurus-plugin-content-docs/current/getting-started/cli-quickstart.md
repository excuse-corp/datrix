...
sidebar_position: 1
title: CLI 安装
summary: "通过 PyPI 安装 Datrix，并用单条命令启动它，无需源码 checkout"
read_when:
  - 你想直接从 PyPI 安装 Datrix
  - 你希望通过 `dbgpt` CLI 完成交互式配置
---

# CLI 安装

通过 PyPI 安装 Datrix，并使用单条命令启动它 —— 无需检出源码仓库。
...
首次运行时，Datrix 会自动启动一个 **交互式配置向导**，帮助你：
...
启动 Datrix web server。直接运行 `dbgpt start` 等价于 `dbgpt start web`。
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `DBGPT_HOME` | `~/.dbgpt` | 覆盖 Datrix home 目录 |
