...
如果要通过 OpenAI 代理运行 Datrix，需要在 `configs/dbgpt-proxy-openai.toml` 配置文件中填入 OpenAI API Key，或者通过环境变量 `OPENAI_API_KEY` 提供。
...
如果要通过 DeepSeek 代理运行 Datrix，需要在 `configs/dbgpt-proxy-deepseek.toml` 中配置 DeepSeek API Key。
...
如果要通过本地模型运行 Datrix，可以修改 `configs/dbgpt-local-glm.toml` 来指定模型路径和其他参数。
...
在 SQLite 下，你不需要手动创建 Datrix 应用相关的数据表；
默认会自动创建。
...
## 测试数据（可选）
Datrix 默认内置了一部分测试数据，你可以通过以下命令将其加载到本地数据库中进行测试。
- **Linux**
