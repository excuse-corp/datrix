...
sidebar_position: 1
title: 快速安装
summary: "使用 README 中的安装脚本，以最快方式安装 Datrix"
read_when:
  - 你希望以最短路径跑通 Datrix Web UI
...
这是让 Datrix 跑起来的最快方式。安装脚本会帮你准备本地 Datrix 工作目录、生成 provider profile，并给出可直接运行的 webserver 启动命令。

## 推荐：安装脚本

如果你想从零开始，以最短路径获得一个可用的 Datrix Web UI，推荐直接使用安装脚本。
...
:::tip 适用场景
如果你想快速体验 Datrix，而不是手动管理仓库结构和依赖细节，这条路径最合适。
:::
...
如果你已经有一个本地 Datrix 仓库，可以直接复用，而不是重新克隆到 `~/.dbgpt/DB-GPT`。
...
- 在 `~/.dbgpt/DB-GPT` 下放置一个 Datrix checkout（如果没有使用 `--repo-dir`）
- 在 `~/.dbgpt/configs/` 下生成 provider 配置文件
- 在 `~/.dbgpt/` 下准备 Datrix home 目录
